...
export function resolveClientConfigPaths({ packageRoot, canonicalConfig }) {
  const launcher = resolveLauncherConfig({ packageRoot, canonicalConfig });

  return {
    serverName: canonicalConfig.serverName,
    command: launcher.command,
    args: launcher.args,
    mcpBinary: launcher.mcpBinary,
    hostBinary: launcher.hostBinary,
...
export function mergeOpenCodeConfig(existingConfig, resolvedConfig) {
  return {
    ...
    mcp: {
      ...
      [resolvedConfig.serverName]: {
        type: "local",
        command: [resolvedConfig.command, ...(resolvedConfig.args ?? [])],
...
function buildEnvironment(resolvedConfig) {
  ...
  if (resolvedConfig.hostBinary) {
    env.DESKTOP_HOST_BIN = resolvedConfig.hostBinary;
  }
...
function renderCodexBlock(resolvedConfig) {
  ...
  const envLines = Object.entries(environment)
    ...
    .join("\n");
  const argsLines = Array.isArray(resolvedConfig.args) && resolvedConfig.args.length > 0
  ...
  return [
    ...
    `command = "${escapeTomlString(resolvedConfig.command)}"`,
    ...(argsLines ? [argsLines] : []),
    "",
...
function resolveLauncherConfig({ packageRoot, canonicalConfig }) {
  const launcher = canonicalConfig.launcher ?? null;
  if (launcher?.command) {
    return {
      command: launcher.command,
      args: Array.isArray(launcher.args)
        ? launcher.args.filter((value) => typeof value === "string" && value.length > 0)
        : [],
      mcpBinary: null,
      hostBinary: null,
    };
  ...
  return {
    command: path.resolve(packageRoot, canonicalConfig.binaries.mcp),
    args: [],
