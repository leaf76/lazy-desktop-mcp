...
- on macOS and Windows, out-of-policy app, window, and session-scope requests can trigger a local user approval dialog that persists a target-only allowlist overlay (expires after `overlay_max_age_seconds`)
...
- `window_list` / `app_list` with a `query` instead of dumping the full desktop
- `observe_capture` only when selectors fail; it returns an artifact id, not pixels. If it reports `unchanged`, skip OCR/vision. Do not loop full-screen capture plus `vision_describe`.
